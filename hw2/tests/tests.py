...
def run_test_suite(name, ctor):
    if name == "prob 1.4B":
        ...
        prob2E = unittest.TestSuite()
        for test in ["testNegRegUpdate", "testPosRegUpdate"]:
            prob2E.addTest(TestLogReg(test, ctor))
        assert unittest.TextTestRunner(verbosity=2).run(prob2E).wasSuccessful(), "one or more tests for prob 2E failed"
    else:   
